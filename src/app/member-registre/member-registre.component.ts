...
@Component({
  selector: 'app-member-registre',
  templateUrl: './member-registre.component.html',
  styleUrls: ['./bootstrap.min.css','./member-registre.component.scss']
})
export class MemberRegistreComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if(this.type_mbr!=null){const objectToSubmit: Member = {...this.item, ...this.form.value, type: this.type_mbr};
      ...
      this.memberService.saveMember(objectToSubmit).then(
        data=> {console.log("response recieved"),this.router.navigate(['./member/login'])},
